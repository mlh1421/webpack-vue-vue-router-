<template>
  <div class="">
    <div v-html='body.body' v-images></div>
  </div>
</template>
<script>
  import Vue from 'vue';
  Vue.directive('images',function (el) {
    var images=el.getElementsByTagName('img'),
      ptn=/https?:\/\//g;
    for(var i=0;i<images.length;i++){
      images[i].src='https://images.weserv.nl/?url='+images[i].src.replace(ptn,'');
    }
  });
  export default {
    name:"content",
    ...
    created:function () {
      this.$http.get(this.href).then(function (response) {
        this.body=response.body.CONTENTS;
      })
    },
    methods:{
    }
